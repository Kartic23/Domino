 ...
 * Categoria de jogador que joga sempre a peca de maior valor do seu lote de pecas, caso contrario, passa a sua vez
 * @author Daniel Fernandes
 * @author Diogo Zacarias
 * @author Kartic Premgi
 * @version 1.0
 */
public class Maximo extends Jogador{

    /**
     * Construtor da subclasse Maximo
     * @param name representa o nome do Jogador
     */
    public Maximo(String name) {
        super(name);
    }

    /**
     * Metodo que calcula o indice da peca com maior valor de um Arraylist recebido
     * @param jogaveis Arraylist que representa as pecas jogaveis
     * @return indice da peca com maior valor do Arraylist
     */
    public int maximo_entre_as_pecas(ArrayList<Peca> jogaveis){
        int maximo = jogaveis.get(0).total_peca();
        int indice = 0;
        for (int i = 1; i < jogaveis.size(); i++) {
            if (maximo < jogaveis.get(i).total_peca()){
                maximo = jogaveis.get(i).total_peca();
                indice = i;
            }
        }
        return indice;
    ...
    /**
     * Metodo que joga a peca de maior valor do seu lote de pecas
    ...
    public void play(Tabuleiro tabuleiro,ArrayList<Coordenadas> coordenadas, ArrayList<Peca> cantos){
        ...
        if (pecas_jogaveis.size() != 0) {
            int indice = maximo_entre_as_pecas(pecas_jogaveis);
